...
test("backend smoke gate has an exact deployment inventory and safe URL boundary", async () => {
    ...
    assert.match(
        ...
        /release:publication && npm run release:fetch && npm run release:source && npm run billing:verify && npm run identity:configure && npm run release:fetch && npm run release:source && npm run deploy:firebase && npm run billing:verify && npm run function-secrets:configure && npm run google:health:bound && npm run backend:smoke(?: &&|$)/u
    );
