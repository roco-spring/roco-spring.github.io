...
After deployment, this read-only gate verifies the exact active six-Function, second-generation Node.js 22 inventory in `europe-west3`, managed HTTPS endpoints, the five public callable IAM bindings, the private Scheduler-only reconciler binding, the enabled OIDC-authenticated five-minute Scheduler target, a successful Scheduler invocation within the preceding 15 minutes, the exact organizer notification channel, and all five alert-policy definitions through Google Cloud control-plane APIs. The recent successful invocation proves that Scheduler could mint its OIDC token and invoke the deployed reconciler; the dependency-health alert separately covers failures that the reconciler handles after invocation.
...
The exact inventory is five public callable Functions—`registerTeam`, `getMyTeam`, `updateMyTeam`, `completeInitialPasswordChange`, and `refreshLeaderboard`—plus the private scheduled `reconcileRegistrations` Function. `refreshLeaderboard` is App Check-protected and secret-free. It projects only public roster fields from Firestore, fetches only bounded allowlisted Spring benchmark pages, and shares a Firestore cache and refresh lease across browser requests.
...
The command finishes all read-only Function, IAM, Scheduler, and channel checks before its first possible write. It fails closed if any extra enabled or paused Scheduler job targets the reconciler, or if any stale/keyless RoCo-managed alert policy falls outside the exact five-key inventory; these require explicit operator review and are never silently deleted. It creates missing managed policies, updates only policies bearing the exact RoCo management labels, refuses to take over a conflicting unmanaged policy, reads all five policies back, and is a no-op on a second healthy run. Its canonical enabled policies are:
...
5. `RoCo leaderboard: public benchmark refresh failed`, a log match on the `refreshleaderboard` service for the single sanitized signal `operation="leaderboardRefresh"`, `status="failed"`, and `severity>=ERROR`. It fires when a genuine upstream/parser refresh attempt fails and validated stale cache is served; application cooldown cache hits do not emit another failure log.

All four log-match policies use a 300-second notification rate limit and 1800-second auto-close. All five policies deliver only to the selected enabled/verified organizer channel. Run `npm run production:runtime:verify` after creating or changing any channel, alert, Function, IAM binding, or Scheduler job. A configuration read-back does not prove email delivery; trigger a controlled test incident and verify the organizer group receives it before checking notification delivery off the release checklist.
